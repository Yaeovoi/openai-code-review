...
public class WeComNotification implements INotification {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WeComNotification.class);

    private final String webhook;

    public WeComNotification(String webhook) {
    ...
    public void send(String logUrl, String project, String branch, String author, String reviewContent) throws Exception {
        ...
        try {
            ...
            logger.info("发送企业微信消息, 请求体: {}", body.toJSONString());
            ...
            if (responseCode == 200) {
                ...
                logger.error("企业微信消息推送失败, HTTP {}", responseCode);
                throw new RuntimeException("企业微信消息推送失败, HTTP " + responseCode);
            }
        } finally {
            if (conn != null) {
                conn.disconnect();
